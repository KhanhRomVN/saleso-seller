...
const PendingOrder: React.FC = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await get<Order[]>("/order/pending");
      setOrders(response);
    } catch (error) {
  ...
  const handleCreateInvoice = async (orderId: string) => {
    try {
      await post("/invoice/accept", { order_id: orderId });
      await fetchOrders(); // Refresh orders after creating invoice
    } catch (error) {
  ...
  const handleRefuseOrder = async (orderId: string) => {
    try {
      await post("/invoice/refuse", { order_id: orderId });
      await fetchOrders(); // Refresh orders after refusing order
    } catch (error) {
